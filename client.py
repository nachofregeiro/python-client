...
def post_data(auth_info, url, data, headers={}, retry_get_auth_token=True):
    print("post_data")
    global token
    if token == None:
    ...
    if response.status_code == 401 and retry_get_auth_token:
        ...
        print("token expired")
        token = None
        return post_data(auth_info, url, params=params, headers=headers, retry_get_auth_token=False)
...
def get_data(auth_info, url, params={}, headers={}, retry_get_auth_token=True):
    ...
    if response.status_code == 401 and retry_get_auth_token:
        ...
        print("token expired")
        token = None
        return get_data(auth_info, url, params=params, headers=headers, retry_get_auth_token=False)
...
def get_paged_results(auth_info, url, page_param_name, page=1, data_field=None, params={}, headers={}):
    results = []

    while True:
        params[page_param_name] = page
        print(params)
        response = get_data(auth_info, url, params=params, headers=headers)
...
if __name__=="__main__":
    ...
    response = get_paged_results(auth_info, "http://localhost:4000/users", "page", params=params)
    ...
    response = get_paged_results_by_date(auth_info, "http://localhost:4000/users", "from", "to", date_from, date_to, minutes_span=20)
